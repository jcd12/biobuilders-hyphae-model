...
def main(args):
    ...
    while i < N and m < M:
        ...
        for hyphae_row in hyphae.itertuples():
            index = hyphae_row[0]
            # check if enough substrate around substrate tip (>0)
            if hyphae_substrate.iloc[index,:].sum():
                angle = hyphae_row.angle
                hi = [hyphae_row.x0, hyphae_row.y0, hyphae_row.x, hyphae_row.y]     # OBS! no need to include x0 and y0 - not used (neither in R code)

                # tip extension value, calculated as the accelerated growth * substrate limitation
                # equation of the Monod type
                extension = tipExtensionMonod(ktip1=ktip1, ktip2=ktip2, Kt=Kt, l=hyphae_row.l,
                ...
                # update data frame
                hyphae.at[index,'x'] = hi[2] + dx
                hyphae.at[index, 'y'] = hi[3] + dy
                hyphae.at[index,'l'] = hyphal_length(hyphae_row)
